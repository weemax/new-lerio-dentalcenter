...
class StripeObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    {
        list($id, $this->_retrieveOptions) = \AmeliaVendor\Stripe\Util\Util::normalizeId($id);
        $this->_opts = \AmeliaVendor\Stripe\Util\RequestOptions::parse($opts);
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = \AmeliaVendor\Stripe\Util\Util::convertToStripeObject($v, $this->_opts);
    ...
    public function updateAttributes($values, $opts = null, $dirty = true, $apiMode = 'v1')
    {
        foreach ($values as $k => $v) {
            ...
            if ('metadata' === $k && \is_array($v)) {
                ...
                $this->_values[$k] = \AmeliaVendor\Stripe\Util\Util::convertToStripeObject($v, $opts, $apiMode);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if (\is_array($value)) {
            if (\AmeliaVendor\Stripe\Util\Util::isList($value)) {
                ...
                return \AmeliaVendor\Stripe\Util\Util::convertToStripeObject($value, $this->_opts)->serializeParameters();
            }
    ...
    public function toArray()
    {
        ...
        return \array_reduce(\array_keys($this->_values), function ($acc, $k) use ($maybeToArray) {
            ...
            $v = $this->_values[$k];
            if (\AmeliaVendor\Stripe\Util\Util::isList($v)) {
