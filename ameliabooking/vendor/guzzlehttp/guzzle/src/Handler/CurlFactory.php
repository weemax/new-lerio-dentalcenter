...
namespace AmeliaVendor\GuzzleHttp\Handler;

use AmeliaVendor\GuzzleHttp\Exception\ConnectException;
use AmeliaVendor\GuzzleHttp\Exception\RequestException;
use AmeliaVendor\GuzzleHttp\Promise as P;
use AmeliaVendor\GuzzleHttp\Promise\FulfilledPromise;
use AmeliaVendor\GuzzleHttp\Promise\PromiseInterface;
use AmeliaVendor\GuzzleHttp\Psr7\LazyOpenStream;
use AmeliaVendor\GuzzleHttp\TransferStats;
use AmeliaVendor\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \AmeliaVendor\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \AmeliaVendor\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \AmeliaVendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \AmeliaVendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
