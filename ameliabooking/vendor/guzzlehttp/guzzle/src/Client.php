...
use AmeliaVendor\GuzzleHttp\Cookie\CookieJar;
use AmeliaVendor\GuzzleHttp\Exception\GuzzleException;
use AmeliaVendor\GuzzleHttp\Exception\InvalidArgumentException;
use AmeliaVendor\GuzzleHttp\Promise as P;
use AmeliaVendor\GuzzleHttp\Promise\PromiseInterface;
 ...
 * @final
 */
class Client implements ClientInterface, \AmeliaVendor\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    /**
     ...
     *   AmeliaVendor\GuzzleHttp\Promise\PromiseInterface that is fulfilled with a
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \AmeliaVendor\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']);
        }
     ...
     *                       request and to the transfer. See \AmeliaVendor\GuzzleHttp\RequestOptions.
     */
     ...
     * @param array               $options Request options to apply. See \AmeliaVendor\GuzzleHttp\RequestOptions.
     */
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        $uri = $this->buildUri(\AmeliaVendor\GuzzleHttp\Psr7\Utils::uriFor($uri), $options);
        if (\is_array($body)) {
    ...
    {
        if (isset($config['base_uri'])) {
            $uri = \AmeliaVendor\GuzzleHttp\Psr7\UriResolver::resolve(\AmeliaVendor\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']), $uri);
        }
     ...
     *
     * @param array $options See \AmeliaVendor\GuzzleHttp\RequestOptions.
     */
    ...
    private function applyOptions(RequestInterface $request, array &$options): RequestInterface
    {
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \AmeliaVendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['decode_content']) && $options['decode_content'] !== true) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \AmeliaVendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (isset($options['body'])) {
            ...
            $modify['body'] = \AmeliaVendor\GuzzleHttp\Psr7\Utils::streamFor($options['body']);
            unset($options['body']);
        }
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = \AmeliaVendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Authorization'], $modify['set_headers']);
        ...
        $request = \AmeliaVendor\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify);
        if ($request->getBody() instanceof \AmeliaVendor\GuzzleHttp\Psr7\MultipartStream) {
