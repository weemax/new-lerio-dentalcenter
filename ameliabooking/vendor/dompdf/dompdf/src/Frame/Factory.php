...
use AmeliaVendor\Dompdf\FrameDecorator\Page as PageFrameDecorator;
use AmeliaVendor\Dompdf\FrameReflower\Page as PageFrameReflower;
use AmeliaVendor\Dompdf\Positioner\AbstractPositioner;
use DOMXPath;
/**
...
class Factory
{
    ...
    public static function decorate_frame(Frame $frame, Dompdf $dompdf, ?Frame $root = null): ?AbstractFrameDecorator
    {
        ...
        $decorator = "AmeliaVendor\\Dompdf\\FrameDecorator\\{$decorator}";
        $reflower = "AmeliaVendor\\Dompdf\\FrameReflower\\{$reflower}";
    ...
    protected static function getPositionerInstance(string $type): AbstractPositioner
    {
        if (!isset(self::$_positioners[$type])) {
            $class = '\AmeliaVendor\Dompdf\Positioner\\' . $type;
