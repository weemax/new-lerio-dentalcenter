...
abstract class PKCS1 extends Progenitor
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (strpos($key, 'BEGIN EC PARAMETERS') && strpos($key, 'BEGIN EC PRIVATE KEY')) {
            ...
            $ecPrivate = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\ECPrivateKey::MAP);
            ...
            $ecParams = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
        ...
        $key = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
        if (is_array($key)) {
            return ['curve' => self::loadCurveByParam($key)];
        }
        $key = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\ECPrivateKey::MAP);
        if (!is_array($key)) {
            throw new \RuntimeException('Unable to perform ASN1 mapping');
        }
    ...
    public static function savePrivateKey(BigInteger $privateKey, BaseCurve $curve, array $publicKey, $secret = null, $password = '', array $options = [])
    {
        ...
        $key = ASN1::encodeDER($key, \AmeliaVendor\phpseclib3\File\ASN1\Maps\ECPrivateKey::MAP);
