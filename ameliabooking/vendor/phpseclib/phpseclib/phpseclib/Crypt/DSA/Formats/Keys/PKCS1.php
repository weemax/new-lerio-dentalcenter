...
use AmeliaVendor\phpseclib3\Crypt\Common\Formats\Keys\PKCS1 as Progenitor;
...
abstract class PKCS1 extends Progenitor
{
    ...
    public static function load($key, $password = '')
    {
        ...
        $key = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\DSAParams::MAP);
        if (is_array($key)) {
            return $key;
        }
        $key = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\DSAPrivateKey::MAP);
        if (is_array($key)) {
            return $key;
        }
        $key = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\DSAPublicKey::MAP);
        if (is_array($key)) {
            return $key;
        }
    ...
    {
        $key = ['p' => $p, 'q' => $q, 'g' => $g];
        $key = ASN1::encodeDER($key, \AmeliaVendor\phpseclib3\File\ASN1\Maps\DSAParams::MAP);
    ...
    public static function savePrivateKey(BigInteger $p, BigInteger $q, BigInteger $g, BigInteger $y, BigInteger $x, $password = '', array $options = [])
    {
        ...
        $key = ASN1::encodeDER($key, \AmeliaVendor\phpseclib3\File\ASN1\Maps\DSAPrivateKey::MAP);
    ...
    {
        $key = ASN1::encodeDER($y, \AmeliaVendor\phpseclib3\File\ASN1\Maps\DSAPublicKey::MAP);
