...
use AmeliaVendor\phpseclib3\Crypt\DES;
use AmeliaVendor\phpseclib3\Crypt\Random;
use AmeliaVendor\phpseclib3\Crypt\RC2;
use AmeliaVendor\phpseclib3\Crypt\RC4;
use AmeliaVendor\phpseclib3\Crypt\TripleDES;
...
abstract class PKCS8 extends PKCS
{
     ...
     *
     * @return \AmeliaVendor\phpseclib3\Crypt\Common\SymmetricKey
     * @param string $algo
     */
    ...
    protected static function load($key, $password = '')
    {
        ...
        $meta = [];
        $decrypted = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\EncryptedPrivateKeyInfo::MAP);
        if (strlen($password) && is_array($decrypted)) {
            ...
            switch ($algorithm) {
                ...
                case 'pbeWithSHAAnd40BitRC4':
                    ...
                    $map = ASN1::asn1map($temp[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\PBEParameter::MAP);
                    $salt = $map['salt'];
                ...
                case 'id-PBES2':
                    ...
                    $temp = ASN1::asn1map($temp[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\PBES2params::MAP);
                    ...
                    if (!$cipher instanceof RC2) {
                        ...
                        $map = ASN1::asn1map($temp[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\RC2CBCParameter::MAP);
                    ...
                    switch ($keyDerivationFunc['algorithm']) {
                        case 'id-PBKDF2':
                            ...
                            $params = ASN1::asn1map($temp[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\PBKDF2params::MAP);
                            if (empty($params['prf'])) {
        ...
        $private = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\OneAsymmetricKey::MAP);
        if (is_array($private)) {
            if ($isPublic) {
        ...
        $public = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\PublicKeyInfo::MAP);
        if (is_array($public)) {
            if ($isPrivate) {
    ...
    protected static function wrapPrivateKey($key, $attr, $params, $password, $oid = null, $publicKey = '', array $options = [])
    {
        ...
        $key = ASN1::encodeDER($key, \AmeliaVendor\phpseclib3\File\ASN1\Maps\OneAsymmetricKey::MAP);
        if (!empty($password) && is_string($password)) {
            ...
            if ($encryptionAlgorithm == 'id-PBES2') {
                ...
                $PBKDF2params = ASN1::encodeDER($PBKDF2params, \AmeliaVendor\phpseclib3\File\ASN1\Maps\PBKDF2params::MAP);
                if (!$crypto instanceof RC2) {
                    $params = ['octetString' => $iv];
                } else {
                    $params = ['rc2ParametersVersion' => 58, 'iv' => $iv];
                    $params = ASN1::encodeDER($params, \AmeliaVendor\phpseclib3\File\ASN1\Maps\RC2CBCParameter::MAP);
                ...
                $params = ASN1::encodeDER($params, \AmeliaVendor\phpseclib3\File\ASN1\Maps\PBES2params::MAP);
                $crypto->setIV($iv);
            } else {
                ...
                $params = ASN1::encodeDER($params, \AmeliaVendor\phpseclib3\File\ASN1\Maps\PBEParameter::MAP);
            }
            ...
            $key = ASN1::encodeDER($key, \AmeliaVendor\phpseclib3\File\ASN1\Maps\EncryptedPrivateKeyInfo::MAP);
    ...
    protected static function wrapPublicKey($key, $params, $oid = null, array $options = [])
    {
        ...
        $key = ASN1::encodeDER($key, \AmeliaVendor\phpseclib3\File\ASN1\Maps\PublicKeyInfo::MAP);
        if (isset($options['binary']) ? $options['binary'] : self::$binary) {
            return $key;
        }
    ...
    public static function extractEncryptionAlgorithm($key)
    {
        ...
        $decoded = self::preParse($key);
        $r = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\EncryptedPrivateKeyInfo::MAP);
        if (!is_array($r)) {
        ...
        if ($r['encryptionAlgorithm']['algorithm'] == 'id-PBES2') {
            ...
            $r['encryptionAlgorithm']['parameters'] = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\PBES2params::MAP);
            ...
            switch ($kdf['algorithm']) {
                case 'id-PBKDF2':
                    ...
                    $kdf['parameters'] = ASN1::asn1map($decoded[0], \AmeliaVendor\phpseclib3\File\ASN1\Maps\PBKDF2params::MAP);
            }
        }
