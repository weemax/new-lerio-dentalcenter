...
class XML extends Parser
{
     ...
     *
     * @var \AmeliaVendor\Sabre\VObject\Document
     */
    protected $root;
    /**
     * Creates the parser.
    ...
    public function setInput($input)
    {
        ...
        if (is_string($input)) {
            ...
            $reader->elementMap['{' . self::XCAL_NAMESPACE . '}period'] = \AmeliaVendor\Sabre\VObject\Parser\XML\Element\KeyValue::class;
            $reader->elementMap['{' . self::XCAL_NAMESPACE . '}recur'] = \AmeliaVendor\Sabre\VObject\Parser\XML\Element\KeyValue::class;
            $reader->xml($input);
            $input = $reader->parse();
        }
        $this->input = $input;
