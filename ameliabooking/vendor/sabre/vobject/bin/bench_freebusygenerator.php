...
if ($argc < 2) {
    echo 'sabre/vobject ', \AmeliaVendor\Sabre\VObject\Version::VERSION, " freebusy benchmark\n";
    echo "\n";
...
$bench->parse->start();
$vcal = \AmeliaVendor\Sabre\VObject\Reader::read(fopen($inputFile, 'r'));
$bench->parse->stop();
$repeat = 100;
...
$bench->fb->start();
for ($i = 0; $i < $repeat; ++$i) {
    $fb = new \AmeliaVendor\Sabre\VObject\FreeBusyGenerator($start, $end, $vcal, $timeZone);
